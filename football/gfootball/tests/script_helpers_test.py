...
class ScriptHelpersTest(unittest.TestCase):
    def generate_replay(self):
        ...
        cfg.update({
            ...
            'players': ['agent:left_players={}'.format(left_players), 'bot:right_players=1', 'lazy:right_players=1'],
    ...
    def compute_hash(self, trace_file):
        ...
        for frame in replay:
            del frame['debug']
            hash_value = zlib.adler32(str(tuple(sorted(frame.items()))).encode(), hash_value)
        return hash_value
    ...
    def test__replay(self):
        ...
        script_helpers.ScriptHelpers().replay(list(dump1)[0], directory=test_tmpdir, config_update=cfg, render=False)
