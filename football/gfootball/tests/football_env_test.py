...
from football.gfootball.env import config
...
from football.gfootball.env import scenario_builder

fast_run = False

def observation_hash(observation, hash_value = 0):
    for obs in observation:
        hash_value = zlib.adler32(str(tuple(sorted(obs.items()))).encode(), hash_value)
    return hash_value
...
class FootballEnvTest(parameterized.TestCase):
    ...
    def test_score_empty_goal(self):
        ...
        for _ in range(120):
            ...
            self.assertGreaterEqual(o['left_team'][o['active']][0], last_o['left_team'][last_o['active']][0] - 0.01)
            last_o = o
    ...
    def test_player_order_invariant(self):
        ...
        cfg = config.Config({'level': 'tests.11_vs_11_hard_deterministic', 'players': players})
        env = football_env.FootballEnv(cfg)
        actions = len(football_action_set.get_action_set(cfg))
    ...
    def test_symmetry(self, seed):
        ...
        queue1 = Queue()
        thread1 = threading.Thread(target=run_scenario, args=(cfg1, seed, queue1, actions))
        thread1.start()
        queue2 = Queue()
        thread2 = threading.Thread(target=run_scenario, args=(cfg2, seed, queue2, actions))
        thread2.start()
        while True:
            o1 = queue1.get()
            o2 = queue2.get()
    ...
    @parameterized.parameters((1, 'left', True), (0, 'right', True),(1, 'left', False), (0, 'right', False))
    ...
    @parameterized.parameters((0, 1, True), (1, -1, True), (0, 1, False),(1, -1, False))
    ...
    def test_render_state_equals_norender(self, seed):
        ...
        queue1 = Queue()
        thread1 = threading.Thread(target=run_scenario, args=(cfg1, seed, queue1, actions, False, False))
        thread1.start()
        queue2 = Queue()
        thread2 = threading.Thread(target=run_scenario, args=(cfg2, seed, queue2, actions, True, False))
        thread2.start()
        while True:
            o1 = queue1.get()
            o2 = queue2.get()
    ...
    def test_get_state_wrapper(self):
        env = football.gfootball.env.create_environment(
            stacked=True,
    ...
    def test_restore_after_reset(self):
        cfg = config.Config({'level': '11_vs_11_competition',})
        env = football_env.FootballEnv(cfg)
        obs = env.reset()
        state = env.get_state()
        env.reset()
