...
from football.gfootball.env import football_env_core
from football.gfootball.env import observation_rotation
...
class FootballEnv(gym.Env):
    ...
    def _construct_players(self, definitions, config):
        ...
        for definition in definitions:
            ...
            try:
                player_factory = importlib.import_module('football.gfootball.env.players.{}'.format(name))
    ...
    #       player.
    def _convert_observations(self, original, player, left_player_position, right_player_position):
        observations = []
    ...
    def _get_actions(self):
        ...
        for player in self._players:
            adopted_obs = self._convert_observations(obs, player, left_player_position, right_player_position)
            ...
            assert len(adopted_obs) == len(a), 'Player provided {} actions instead of {}.'.format(len(a), len(adopted_obs))
            if not player.can_play_right():
                for x in range(player.num_controlled_right_players()):
                    index = x + player.num_controlled_left_players()
                    a[index] = observation_rotation.flip_single_action(a[index], self._config)
    ...
    def step(self, action):
        ...
        score_reward = reward
        if self._agent:
            reward = ([reward] * self._agent.num_controlled_left_players() + [-reward] * self._agent.num_controlled_right_players())
        self._cached_observation = None
        return (self.observation(), np.array(reward, dtype=np.float32), done, {'score_reward': score_reward})
    ...
    def observation(self):
        if not self._cached_observation:
            ...
            if self._agent:
                self._cached_observation = self._convert_observations(self._cached_observation, self._agent, self._agent_left_position, self._agent_right_position)
