...
from football.gfootball.env import wrappers
...
def create_environment(env_name='',
    ...
    assert env_name
    players = [('agent:left_players=%d,right_players=%d' % (number_of_left_players_agent_controls, number_of_right_players_agent_controls))]
    ...
    env = _apply_output_wrappers(env, rewards, representation, channel_dimensions, (number_of_left_players_agent_controls + number_of_right_players_agent_controls == 1), stacked)
    return env
...
# Returns:
#   Google Research Football environment.
def create_remote_environment(username,
                              token,
                              model_name='',
                              track='',
                              stacked=False,
    ...
    from football.gfootball.env import remote_football_env
    env = remote_football_env.RemoteFootballEnv(username, token, model_name=model_name, track=track, include_rendering=include_rendering)
    env = _apply_output_wrappers(env, rewards, representation, channel_dimensions,env._config.number_of_players_agent_controls() == 1, stacked)
    return env
