...
from football.gfootball.env import controller_base
from football.gfootball.env import football_action_set
from football.gfootball.env import event_queue

BUTTON_TO_ACTIONS = {
        0: [football_action_set.action_short_pass, football_action_set.action_pressure],
        1: [football_action_set.action_shot, football_action_set.action_team_pressure],
        2: [football_action_set.action_high_pass, football_action_set.action_sliding],
        3: [football_action_set.action_long_pass, football_action_set.action_keeper_rush],
    ...
    def take_action(self, observations):
        ...
        for event in event_queue.get('gamepad', self._index):
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                for a in actions:
                    active_buttons[a] = 1
            if (event.type == pygame.JOYAXISMOTION and event.axis == 5 and event.value > 0):
