...
python3 -m football.gfootball.play_game \
...
from football.gfootball.examples import models  
...
class Player(player_base.PlayerBase):
    ...
    def __init__(self, player_config, env_config):
        ...
        _load_variables(player_config['checkpoint'], self._sess, prefix=self._player_prefix + '/')
...
def _load_variables(load_path, sess, prefix='', remove_prefix=True):
    ...
    variables = [v for v in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES) if v.name.startswith(prefix)]
...
class DummyEnv(object):
    ...
    def __init__(self, action_set, stacking):
        self.action_space = gym.spaces.Discrete(len(football_action_set.action_set_dict[action_set]))
        self.observation_space = gym.spaces.Box(0, 255, shape=[72, 96, 4 * stacking], dtype=np.uint8)
