...
class DumpConfig(object):
    def __init__(self, max_length=200, max_count=1, snapshot_delay=0, min_frequency=10):
...
class TextWriter(object):
    ...
    def write(self, text, scale_factor=1):
        ...
        lineType = 1
        cv2.putText(self._frame, text, textPos, font, fontScale, self._color, lineType)
...
def get_frame(trace):
    ...
    writer = TextWriter(frame, trace['ball'][0], trace['ball'][1], field_coords=True, color=(255, 0, 0))
    writer.write('B')
    for player_idx, player_coord in enumerate(trace['left_team']):
        writer = TextWriter(frame, player_coord[0], player_coord[1], field_coords=True, color=(0, 255, 0))
        letter = 'H'
        if 'active' in trace and player_idx in trace['active']:
            letter = 'X'
        elif 'left_agent_controlled_player' in trace and player_idx in trace['left_agent_controlled_player']:
            letter = 'X'
        writer.write(letter)
    for player_idx, player_coord in enumerate(trace['right_team']):
        writer = TextWriter(frame, player_coord[0], player_coord[1], field_coords=True, color=(255, 255, 0))
        letter = 'A'
        if 'opponent_active' in trace and player_idx in trace['opponent_active']:
            letter = 'Y'
        elif 'right_agent_controlled_player' in trace and player_idx in trace['right_agent_controlled_player']:
            letter = 'Y'
        writer.write(letter)
    return frame
...
def write_dump(name, trace, config):
    ...
    if config['write_video']:
        ...
        if config['video_quality_level'] == 2:
            ...
            frame_dim = (800, 450)
        video = cv2.VideoWriter(temp_path, fcc, constants.PHYSICS_STEPS_PER_SECOND / config['physics_steps_per_frame'], frame_dim)
        frame_cnt = 0
        time = trace[0]._time
        for o in trace:
            frame_cnt += 1
            ...
            if config['display_game_stats']:
                ...
                for i in range(len(sticky_actions)):
                    if sticky_actions[i]._directional:
                        ...
                        writer.write('%s: %d' % (sticky_actions[i]._name, o[sticky_actions_field][0][i]))
                writer.write('DIRECTION: %s' % ('NONE' if active_direction is None else active_direction._name))
...
class ObservationProcessor(object):
    def __init__(self, config):
        ...
        self._frame = 0
        self._dump_config = {}
        self._dump_config['score'] = DumpConfig(max_length=200, max_count=(100000 if config['dump_scores'] else 0), min_frequency=600, snapshot_delay=10)
        self._dump_config['lost_score'] = DumpConfig(max_length=200, max_count=(100000 if config['dump_scores'] else 0), min_frequency=600, snapshot_delay=10)
        self._dump_config['episode_done'] = DumpConfig(max_length=10000, max_count=(100000 if config['dump_full_episodes'] else 0))
    ...
    def write_dump(self, name):
        ...
        config._file_name = '{2}/{0}_{1}'.format(name, datetime.datetime.now().strftime('%Y%m%d-%H%M%S%f'), self._dump_directory)
