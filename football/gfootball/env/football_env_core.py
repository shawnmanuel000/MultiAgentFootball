...
from football.gfootball.env import config as cfg
from football.gfootball.env import constants
from football.gfootball.env import football_action_set
from football.gfootball.env import observation_processor
...
class FootballEnvCore(object):
    ...
    def _reset(self, animations, inc):
        ...
        assert (self._env.state == GameState.game_created or self._env.state == GameState.game_running or self._env.state == GameState.game_done)
        self._steps_time = 0
        self._step = 0
    ...
    def _rendering_in_use(self):
        ...
        if not self._use_rendering_engine:
            assert not _active_rendering, ('Environment does not support multiple rendering instances in the same process.')
    ...
    def step(self, action, extra_data={}):
        assert self._env.state != GameState.game_done, ('Cant call step() once episode finished (call reset() instead)')
        assert self._env.state == GameState.game_running, ('reset() must be called before step()')
        action = [football_action_set.named_action_from_action_set(self._action_set, a) for a in action]
        self._step_count += 1
        ...
        if self._env.waiting_for_game_count > 20:
            ...
            action = [football_action_set.action_short_pass] * (self._env.config.left_agents + self._env.config.right_agents)

        assert len(action) == (self._env.config.left_agents + self._env.config.right_agents)
        debug = {}
        debug['action'] = action
        action_index = 0
        ...
        if (self._env.config.end_episode_on_out_of_play and
            self._trace.len() > 0 and self._observation['game_mode'] != int(libgame.e_GameMode.e_GameMode_Normal) and
            self._trace[-1]['observation']['game_mode'] == int(libgame.e_GameMode.e_GameMode_Normal)):
            self._env.state = GameState.game_done

        # End episode when team possessing the ball changes.
        if (self._env.config.end_episode_on_possession_change and self._trace.len() > 0 and self._observation['ball_owned_team'] != -1):
            ...
            while prev_posession_id > 0 and self._trace[prev_posession_id]['observation']['ball_owned_team'] == -1:
                prev_posession_id -= 1
            prev_posession = self._trace[prev_posession_id]['observation']['ball_owned_team']
            if prev_posession != -1 and prev_posession != current_posession:
                self._env.state = GameState.game_done

        reward = (self._observation['score'][0] - self._observation['score'][1] - previous_score_diff)
        if reward == 1:
    ...
    def _retrieve_observation(self):
        ...
        if self._env.game_config.render:
            ...
            frame = np.reshape(np.concatenate([frame[:, :, 0], frame[:, :, 1], frame[:, :, 2]]), [3, 720, 1280])
            frame = np.transpose(frame, [1, 2, 0])
            frame = np.flip(frame, 0)
            result['frame'] = frame
        result['ball'] = np.array([info.ball_position[0], info.ball_position[1], info.ball_position[2]])
        # Ball's movement direction represented as [x, y] distance per step.
        result['ball_direction'] = np.array([info.ball_direction[0], info.ball_direction[1], info.ball_direction[2]])
        # Ball's rotation represented as [x, y, z] rotation angle per step.
        result['ball_rotation'] = np.array([info.ball_rotation[0], info.ball_rotation[1], info.ball_rotation[2]])
        ...
        for i in range(self._env.config.left_agents):
            result['left_agent_controlled_player'].append(info.left_controllers[i].controlled_player)
            result['left_agent_sticky_actions'].append(np.array(self.sticky_actions_state(True, i), dtype=np.uint8))
        for i in range(self._env.config.right_agents):
            result['right_agent_controlled_player'].append(info.right_controllers[i].controlled_player)
            result['right_agent_sticky_actions'].append(np.array(self.sticky_actions_state(False, i), dtype=np.uint8))
    ...
    def observation(self):
        ...
        assert (self._env.state == GameState.game_running or self._env.state == GameState.game_done), ('reset() must be called before observation()')
    ...
    def get_state(self, to_pickle):
        assert (self._env.state == GameState.game_running or self._env.state == GameState.game_done), ('reset() must be called before get_state()')
        pickle = six.moves.cPickle.dumps(to_pickle)
        return self._env.get_state(pickle)

    def set_state(self, state):
        assert (self._env.state == GameState.game_running or self._env.state == GameState.game_done), ('reset() must be called before set_state()')
