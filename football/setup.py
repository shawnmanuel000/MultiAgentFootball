...
class CustomBuild(build_ext):
	...
	def run(self):
		...
		if os.system('cp -r third_party/fonts ' + dest_dir):
			raise OSError("Google Research Football: Could not copy fonts to %s." % (dest_dir))

		try:
			use_prebuilt_library = int(os.environ.get("GFOOTBALL_USE_PREBUILT_SO", "0"))
		except ValueError:
			raise ValueError("Could not parse GFOOTBALL_USE_PREBUILT_SO environment variable as int. Please set it to 0 or 1")
		...
		if use_prebuilt_library:
			...
			elif (os.system('gfootball/build_game_engine.sh') or os.system("cp third_party/gfootball_engine/_gameplayfootball.so " + dest_dir)):
